...
export default {
  ...
  methods: {
    // 把发请求封装成一个方法 根据参数不同返回不同的数据进行展示
    getData() {
      this.$store.dispatch("getSearchList", this.searchParams);
    },
    // 删除面包屑
    removeCategoryName() {
      // 把带给服务器的参数置空了 需要重新向服务器发送请求
      // 带给服务器的参数是可有可无的 如果属性值为空的字符串还是会把相应的字段带给服务器
      // 把这个字段设置为undefined 当前这个字段不会带给服务器更新 优化服务器
  ...
  // 再次发送路由请求时 要把两个请求组合 即“筛选” 需要重新合并请求对象
  watch: {
    $route() {
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      this.getData();
